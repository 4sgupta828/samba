...
class Simulation:
    ...
    def save_artifacts_and_shutdown(self):
        ...
        # Reports are now generated by TrainingFailureInjector in generate_dataset.py
        # and tracked via CausalityTracker
        if not self.reports:
    ...
    def _determine_simulation_name(self) -> str:
        ...
        Returns:
            Name indicating the scenario type: "Training Episode" or "Deployment Scenario"
        """
        # Check if there are any failure events in ScenarioEventTracker
        from src.core.scenario_events import ScenarioEventTracker
        scenario_tracker = ScenarioEventTracker()
        has_failure_events = len([e for e in scenario_tracker.events if 'failure' in e.category or 'infrastructure' in e.category]) > 0

        has_deployment_scenario = self.config.get('deployments') is not None

        if has_failure_events and has_deployment_scenario:
            return "Training Episode (Deployment + Failure)"
        elif has_failure_events:
            return "Training Episode (Failure Injection)"
