...
def load_semantic_map(episode_path: str) -> Optional[Dict]:
    """
    Load semantic overlay map from episode directory (if available).

    The semantic map contains domain-specific service names, resource profiles,
    and deterministic request flows generated by Claude AI.

    Args:
        episode_path: Path to episode directory

    Returns:
        Dictionary with semantic overlay (domain, services, request_flows) or None if not available
    """
    semantic_file = os.path.join(episode_path, "semantic_map.json")
    ...
    try:
        ...
        print(f"Warning: Failed to load semantic map: {e}")
        return None
...
def load_metrics(data_dir: str) -> pd.DataFrame:
    ...
    with open(metrics_file, 'r') as f:
        for line in f:
            ...
            # Flatten structure
            # Handle different component ID label formats:
            # - component.id: Used by most components (services, pods, databases, etc.)
            # - node.id: Used by ComputeNode infrastructure metrics
            # - metric name prefix: Used by workload generator (workload.*)
            component_id = data['labels'].get('component.id')
            if not component_id:
                # Check for node.id (used by ComputeNode metrics)
                component_id = data['labels'].get('node.id')
            if not component_id:
                # Check if this is a workload metric (workload.*)
                if data['name'].startswith('workload.'):
                    component_id = 'workload'
                else:
                    component_id = 'global'
...
def load_episode(episode_id: str, data_run_path: str) -> Dict:
    ...
    # Apply semantic names to topology graph (if semantic map is available)
    if semantic_map and 'services' in semantic_map:
        for node_id in topology_graph.nodes():
            if node_id in semantic_map['services']:
                semantic_info = semantic_map['services'][node_id]
                # Add semantic name and profile as node attributes
                topology_graph.nodes[node_id]['semantic_name'] = semantic_info.get('name', node_id)
                topology_graph.nodes[node_id]['resource_profile'] = semantic_info.get('profile', 'standard')
                # Also add domain for reference
                topology_graph.nodes[node_id]['domain'] = semantic_map.get('domain', 'unknown')

    # Add synthetic workload generator node
    # The workload generator is not in the topology.json but generates requests
    topology_graph.add_node('workload', type='WorkloadGenerator', role='client', semantic_name='WorkloadGenerator')
